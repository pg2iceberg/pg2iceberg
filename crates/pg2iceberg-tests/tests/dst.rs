...
//!
//! Then asserts the plan §9 invariants that don't require the materializer:
//!
...
//!
//! Workload generator includes `CrashAndRestart` to model pipeline-process
//! crashes between flushes; idempotent replay must keep all four invariants.
...
enum Step {
    ...
    /// from the slot's `restart_lsn`. The coord and blob_store survive
    /// (those represent durable storage).
    CrashAndRestart,
}
...
fn check_invariants(h: &mut DstHarness) -> Result<(), String> {
    // Reach quiescence: drain WAL, flush, ack.
    h.drive();
    h.flush_and_ack();
